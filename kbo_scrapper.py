...
for current in [last_gameday + timedelta(days=x) for x in range(1, int((date.today() - last_gameday).days) + 1)]:
    ...
    if current != date.fromisoformat(driver.find_element(By.CLASS_NAME, 'date-txt').text[:-3].replace('.', '-')):
        continue
    game_df = pd.concat([game_df, pd.DataFrame([[date.fromisoformat(x.get_attribute('g_dt')), x.get_attribute('away_nm'), x.get_attribute('home_nm'), int(x.find_element(By.XPATH, './/div[@class="team away"]/div[contains(@class, "score")]').text), int(x.find_element(By.XPATH, './/div[@class="team home"]/div[contains(@class, "score")]').text), x.find_element(By.XPATH, './/div[@class="score win"]/preceding-sibling::div[@class="emb"]/img').get_attribute('alt') if x.find_elements(By.CLASS_NAME, 'win') else 'draw'] for x in driver.find_elements(By.XPATH, '//li[@class="game-cont end"]')], columns = game_df.columns)])
